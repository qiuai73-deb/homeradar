...
#    },
    ...
    # --- 格隆汇快讯（只抓「重要 / 红色」新闻）---
    # 说明：/live 是 SPA，点「重要」按钮只是前端过滤（隐藏 level!=1 的条目），
    #       并不会改变接口返回。重要(红色)新闻的判定字段是 level==1。
    #       因此这里直接拉取接口，并只保留 level==1 的重要新闻。
    "gelonghui": {
        "name": "gelonghui",
        "name_cn": "格隆汇快讯",
        "url": "https://www.gelonghui.com/api/dtb/list",
        "type": "gelonghui",
...
def fetch_source(key, config):
    ...
    try:
        if config["type"] == "rss":
            ...
            print(f"  [格隆汇 API] HTTP 状态码: {response.status_code}")
            response.raise_for_status()

            try:
                data = response.json()
            except ValueError:
                print("  ❌ [格隆汇 API] 返回内容不是 JSON：")
                print(f"  {response.text[:300]}")
                return []

            if not isinstance(data, dict):
                print("  ❌ [格隆汇 API] 返回 JSON 不是对象")
                return []

            items = data.get("result") or []
            print(f"  [格隆汇 API] result 数量: {len(items)}")

            # 关键：只保留「重要」新闻（level == 1）
            important_items = [
                it for it in items
                if isinstance(it, dict) and it.get("level") == 1
            ]
            print(f"  [格隆汇 API] 其中重要(level=1)新闻: {len(important_items)} 条")

            from datetime import timedelta
            beijing_tz = timezone(timedelta(hours=8))

            for item in important_items[:MAX_ARTICLES]:
                ...
                nid = item.get("id")
                # link 字段通常为空，退回用快讯详情页地址（含 id，便于去重）
                link = item.get("link") or (
                    f"https://www.gelonghui.com/live/{nid}" if nid else ""
                )

                # createTime 为 Unix 秒，转北京时间
                published = ""
                ct = item.get("createTime")
                if ct not in (None, "", 0):
                    try:
                        published = datetime.fromtimestamp(
                            int(float(ct)), tz=beijing_tz
                        ).strftime("%Y-%m-%d %H:%M:%S")
                    except (TypeError, ValueError, OSError, OverflowError):
                        published = str(ct)

                # 附带关联股票 / 话题标签，方便后续 AI 分析
                stocks = item.get("stockList") or []
                stock_tags = " ".join(
                    str(s.get("name") or s.get("stockName") or "")
                    for s in stocks if isinstance(s, dict)
                )
                summary = content
                if stock_tags:
                    summary = f"{content}\n[关联股票] {stock_tags}".strip()
            ...
            print(f"  [格隆汇 API] 成功解析重要新闻: {len(articles)} 条")
